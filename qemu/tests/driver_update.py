...
def run(test, params, env):
    ...
    def get_installation_log(vm, session=None):
        ...
        result_dir = test.resultsdir

        for log_file in driver_logs:
            logging.info("Copy debug file %s from guest to host" % log_file)
            try:
                vm.copy_files_from(log_file, result_dir)
            except Exception, detail:
                logging.error("Didn't find file %s in guest: %s" %
                              (log_file, detail))
    ...
    def get_driver_from_cdrom(driver_name, session):
        ...
        guest_name_alias = params.get("guest_alias")
        guest_name = params.get("guest_name")
        guest_list = {}
        for i in guest_name_alias.split(","):
            guest, guest_alias = i.split(":")
            guest_list[guest] = guest_alias
        guest_n = guest_list[guest_name]
        virtio_win_disk = utils_misc.get_win_disk_vol(session, device_key)
        if not virtio_win_disk:
            raise error.Error("Didn't fine virtio-win iso in guest")

        driver_path = "%s:\%s\%s" % (virtio_win_disk, driver_name, guest_n)
        ...
        inf_file = output.split()[0]
        inf_file = "%s\%s" % (driver_path, inf_file)
